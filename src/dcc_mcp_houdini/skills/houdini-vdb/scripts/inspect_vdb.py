"""Bounded VDB readback: grid names, primitive counts and bounds without cooking."""
...
def inspect_vdb(node_path: str, max_names: int = 16) -> dict:
    try:
        import hou
    except ImportError:
    ...
    try:
        ...
        if geometry is None:
            return skill_success(
                "Inspected VDB node without cooked geometry",
                node=summary,
                geometry_available=False,
                volume_names=[],
                volume_names_available=False,
                prim_count=None,
                point_count=None,
                bounding_box=None,
        ...
        readback = volume_readback(geometry, max_names=max_names)
        return skill_success(
            "Inspected VDB node",
            node=summary,
            geometry_available=True,
            volume_names=readback["volume_names"],
            volume_count=len(readback["volume_names"]),
            volume_names_available=readback["names_available"],
        ...
        return skill_exception(exc, message="Failed to inspect VDB node")


@skill_entry
def main(**kwargs):
    return inspect_vdb(**kwargs)
